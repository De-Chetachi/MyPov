...
const MyPovBlog = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      setPosts(postsData);
      // Simulate not being logged in initially
      setCurrentUser(null);
    } catch (error) {
...
// Post Card Component for Home View
const PostCard = ({ post, currentUser, onViewPost, onLike, onComment }) => {
    ...
    <article className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden">
      ...
      <div className="p-6 pb-4">
        ...
        <div 
          ...
          className="cursor-pointer hover:bg-gray-50 -mx-6 px-6 py-2 rounded-xl transition-colors"
        >
          <h2 className="text-2xl font-bold text-gray-900 mb-3 leading-tight hover:text-indigo-600 transition-colors">
            {post.title}
          </h2>
        ...
        <div className="px-8 py-6 border-t border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-8">
              ...
              >
                <Heart className={`w-6 h-6 ${post.likedBy?.includes(currentUser?._id) ? 'fill-current' : ''}`} />
                <span className="font-medium">{post.likes} {post.likes === 1 ? 'Like' : 'Likes'}</span>
              </button>
              
              <button
                onClick={() => setShowComments(!showComments)}
                className="flex items-center space-x-3 text-gray-500 hover:text-indigo-500 transition-colors"
              >
            ...
            {currentUser && currentUser._id === post.author._id && (
              <div className="flex items-center space-x-2">
                <button className="p-3 text-gray-500 hover:text-indigo-500 rounded-full hover:bg-gray-100">
                  <Edit3 className="w-5 h-5" />
                </button>
                <button className="p-3 text-gray-500 hover:text-red-500 rounded-full hover:bg-gray-100">
          ...
          <div className="p-8">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex justify-end mt-3">
                      <button
                        ...
                        className="flex items-center space-x-2 px-6 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
            ...
            <div className="space-y-6">
              {comments.map((comment) => (
                <div key={comment._id} className="flex items-start space-x-4 p-4 hover:bg-gray-50 rounded-xl transition-colors">
              ...
              {currentUser ? (
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full text-center py-2 text-gray-700 hover:text-indigo-600 font-medium border border-gray-300 rounded-full"
                  >
                    Login
                  </button>
                  <button
                    ...
                    className="w-full flex items-center justify-center bg-indigo-600 text-white px-4 py-2 rounded-full"
                  >
                    Sign Up
                  </button>
                </div>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {currentView === 'home' ? (
          ...
          <PostDetailView 
            post={viewingPost}
            currentUser={currentUser}
            comments={comments[viewingPost?._id] || []}
            newComment={newComment}
            setNewComment={setNewComment}
            onAddComment={() => handleAddComment(viewingPost._id)}
            onLike={() => handleLike(viewingPost._id)}
